...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        create_directories(self, self.split, args)
        self.first = True
    ...
    def update_paths(self, args):
        ...
        self.LIDAR_PATH = []
        self.LIDAR_SEG_PATH = []
        self.LIDAR_PATH_SWEEPS = []
        self.LIDAR_SEG_SWEEPS_PATH = []
        ...
        for p in range(fdv["sets"]):
            ...
            self.LIDAR_PATH_SWEEPS.append(os.path.join(
                f'../dataset/nus/LiDAR_p{p}_samples/',
                'sweeps/LIDAR_TOP/n008-2018-08-01-00-00-00-0400__LIDAR_TOP__15{0:014}.pcd.bin'))

            self.LIDAR_SEG_SWEEPS_PATH.append(os.path.join(
                f'../dataset/nus/LiDAR_p{p}_samples/', 'sweeps_lidarseg/v1.0-trainval/3{0:031}_lidarseg.bin'))
    ...
    def _check_distance_from_last(self):
        ...
        if yylen > 401:
            distance = self.distance_arrays(curr_location, self.prev_location[yylen - 400])
        else:
    ...
    def _save_training_files(self, data, datapoints, point_cloud, lidar_tags, i=None):
        ...
        if len(datapoints) > 0:
            ...
            for i_frame in range(self.lidar.sets):
                ...
                lidar_sweeps_frame.append(self.LIDAR_PATH_SWEEPS[i_frame].format(self.save_no))

                seg_sweeps_frame.append(self.LIDAR_SEG_SWEEPS_PATH[i_frame].format(self.save_no))  # sweeps lidarseg
            ...
            if self.save_no % 10 == 0:
                ...
                for i_f in range(self.lidar.sets):
                    save_lidar_data_sweeps(lidar_sweeps_frame[i_f], point_cloud[i_f], LIDAR_HEIGHT_POS, "bin")
                    save_lidarseg_tags(seg_sweeps_frame[i_f], lidar_tags[i_f], "bin")
        ...
        while self.manager._running:
            ...
            if timestamp:
                ...
                if first:
                    ...
                    no = re.search(r'\d+', self.manager.scenario_tree.name)
                    self.save_no = int(no.group()) * 1_0000_0000
...
def in_roi(bbox: np.ndarray, limits: np.ndarray):
    ...
    #     return True
    if min(b0, b1, b2, b3, b4, b5, b6, b7) < float(66.7 * 66.7):
        return True
    else:
        return False
