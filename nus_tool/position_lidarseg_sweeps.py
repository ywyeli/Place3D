...
# write json file
def nuread(pathh, path_position, root_path):
    ...
    # sample_data_token
    sample_data_token_lidar_top = 30000000000000000000000000000000

    sample_data_token_cam_front = 30100000000000000000000000000000

    sample_data_token_cam_back = 30200000000000000000000000000000

    sample_data_token_cam_front_left = 30300000000000000000000000000000

    sample_data_token_cam_front_right = 30400000000000000000000000000000

    sample_data_token_cam_back_left = 30500000000000000000000000000000

    sample_data_token_cam_back_right = 30600000000000000000000000000000
    ...
    for sum_no in trange(1000):
        # print(sum_no)
        check_no = 1500_0000_0000_0000 + (40 * (it + 1)) * 10 + n - 10
        check_2 = pathh + '/' + str(check_no) + '.txt'

        while os.path.exists(check_2) == 0:
            n += 1_0000_0000
            it = 0
            check_no = 1500000000000000 + (40 * (it + 1)) * 10 + n - 10
            check_2 = pathh + '/' + str(check_no) + '.txt'
        ...
        if os.path.exists(check_2):
            for ic in range(40):
                check.append(1500000000000000 + (40 * it + ic) * 10 + n)
            it += 1
        else:
            print('Error!')
    ...
    for ie in trange(len(check)):
        ...
        if os.path.exists(label_2):
            ...
            go_x, go_y, go_z, go_roll, go_pitch, go_yaw = position_data[0]

            ego_yaw = - float(go_yaw)
            ego_x = float(go_x)
            ego_y = - float(go_y)
            ego_z = float(go_z)
                    ...
                    if sample_token % 40 == 1:
                        ...
                        lidarseg_json.append(
                            {'token': str(sample_data_token_lidar_top),
                             ...
                             'filename': "lidarseg/v1.0-trainval/" + "3{0:031d}".format(int(timestamp)-1500_0000_0000_0000) + "_lidarseg.bin"}
                        )
                    elif sample_token % 40 == 0:
                        for tk in [9, 8, 7, 6, 5, 4, 3, 2, 1, 0]:
                            if tk == 0:
                                is_key_frame = True
                                filename = 'samples/' + s + '/n008-2018-08-01-00-00-00-0400__' + s + '__' + timestamp + forma
                                prev_ego_pose_token = str(ego_pose_token - 1 - tk)
                                next_ego_pose_token = ''
                                lidarseg_json.append(
                                    {'token': str(sample_data_token_lidar_top),
                                     'sample_data_token': str(sample_data_token_lidar_top),
                                ...
                                is_key_frame = False
                                filename = 'sweeps/' + s + '/n008-2018-08-01-00-00-00-0400__' + s + '__' + str(int(timestamp) - tk) + forma
                                prev_ego_pose_token = str(ego_pose_token - 1 - tk)
                                next_ego_pose_token = str(ego_pose_token + 1 - tk)
                            sample_data_json.append(
                                {'token': str(ego_pose_token - tk),
                                 'sample_token': str(sample_token),
                                 'ego_pose_token': str(ego_pose_token - tk),
                                 'calibrated_sensor_token': calibrated_sensor_token,
                                 'timestamp': int(timestamp) - tk,
                                 'fileformat': fileformat,
                                 'is_key_frame': is_key_frame,
                                 'height': height,
                                 'width': width,
                                 'filename': filename,
                                 'prev': prev_ego_pose_token,
                                 'next': next_ego_pose_token}
                            )
                            position_v = path_position + '/' + str(int(timestamp) - tk) + '.txt'
                            p_vd = openreadtxt(position_v)
                            g_x, g_y, g_z, g_roll, g_pitch, g_yaw = p_vd[0]
                            gg_yaw = - float(g_yaw)
                            gg_x = float(g_x)
                            gg_y = - float(g_y)
                            gg_z = float(g_z)
                            ego_pose_json.append(
                                {'token': str(ego_pose_token - tk),
                                 'timestamp': int(timestamp) - tk,
                                 'rotation': [cos(gg_yaw / 2), 0, 0, sin(gg_yaw / 2)],
                                 'translation': [gg_x, gg_y, gg_z]}
                            )
                    else:
                        for tk in [9, 8, 7, 6, 5, 4, 3, 2, 1, 0]:
                            if tk == 0:
                            ...
                            sample_data_json.append(
                                {'token': str(ego_pose_token - tk),
                                 ...
                                 'width': width,
                                 'filename': filename,
                                 'prev': str(ego_pose_token - 1 - tk),
                                 'next': str(ego_pose_token + 1 - tk)}
                            )
                            position_v = path_position + '/' + str(int(timestamp) - tk) + '.txt'
                            p_vd = openreadtxt(position_v)
                            g_x, g_y, g_z, g_roll, g_pitch, g_yaw = p_vd[0]
            ...
            sample_data_token_lidar_top = sample_data_token_lidar_top + 10
            sample_data_token_cam_front = sample_data_token_cam_front + 10
            sample_data_token_cam_back = sample_data_token_cam_back + 10
            sample_data_token_cam_front_left = sample_data_token_cam_front_left + 10
            sample_data_token_cam_front_right = sample_data_token_cam_front_right + 10
            sample_data_token_cam_back_left = sample_data_token_cam_back_left + 10
            sample_data_token_cam_back_right = sample_data_token_cam_back_right + 10
            ...
            for data in datas:
                ...
                tra_2 = [float(data[13]), - float(data[11]), 1.8 - float(data[12]) + float(data[8]) / 2],
                ...
                if data[0] == 'pedestrian':
                    ...
                    size1 = [float(data[9]), float(data[10]), float(data[8]) + 0.6]
                    translation_1[2] += 0.3
                else:
    ...
    f = open(root_path + 'v1.0-trainval/lidarseg.json', 'w')
...
if __name__ == '__main__':
    ...
    pat = root_path + "training/label_2"
    pat_position = root_path + "training/position"
    os.makedirs(root_path + "v1.0-trainval", exist_ok=True)
    nuread(pat, pat_position, root_path)
