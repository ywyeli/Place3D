...
import time
import math
from position_3d import get_FOV_from, get_T_from, get_R_from, get_q, quaternion_multiply
...
def create_lidarseg_category_json(lidarseg_category_json):
    ...
    f = open(root_path + 'v1.0-trainval/lidarseg_category.json', 'w')
...
def nuread(pathh, path_position, path_timestamp, root_path, scene_numbers, args):
    ...
    for ie in trange(len(check)):
        ...
        if os.path.exists(label_2):
            ...
            for data in datas:
                instance_token_h = 'it' + str(file_no)[2:8] + '{0:08d}'.format(scene_name) + '{0:08d}'.format(
                    int(data[-2]))
